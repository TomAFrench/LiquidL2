...
import ipfsApi = require("ipfs-http-client");
...
async function pushDirectoryToIpfs(path): Promise<{ cid?: string }> {
  try {
...
async function publishHashToIpns(ipfsHash: string): Promise<{ name?: string }> {
  try {
...
function nodeMayAllowPublish(ipfsClient: ipfsApi): boolean {
