...
contract WithdrawalVault is Ownable {
    ...
    function borrow(
        IERC20 asset,
        IAaveCollateralVaultProxy aaveCollateralVaultProxy,
    ...
    function repayLoan(
        ...
        address collateralVault,
        uint256 amount
    ) external onlyOwner returns (uint256 repaymentAmount) {
     ...
     * @param withdrawalProof - the proof object which allows the WithdrawalVault to claim from the rootChainManager
     * @return withdrawalAmount - the amount of asset which the vault has gained through the withdrawal
     */
    function claimFunds(
        IERC20 asset,
        IRootChainManager maticRootChainManager,
        bytes calldata withdrawalProof
    ) external onlyOwner returns (uint256 withdrawalAmount) {
