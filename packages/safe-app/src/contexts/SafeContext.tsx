...
import { MaticNetworks, maticNetworks } from "../types";
...
function SafeProvider({ children }: Props) {
  ...
  const onSafeInfo = useCallback(
    ({ safeAddress, network, ethBalance }: SafeInfo) => {
      if (!maticNetworks.includes(network.toLowerCase() as MaticNetworks)) {
        throw Error("Sablier doesn't support this network");
      }
      setSafeInfo({
        safeAddress,
        network: network.toLowerCase() as MaticNetworks,
        ethBalance,
      });
    },
    [setSafeInfo],
...
export const useSafeNetwork = (): MaticNetworks | undefined => {
  const { network } = useSafeInfo() || {};
  return network as MaticNetworks;
};
